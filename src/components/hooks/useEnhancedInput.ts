...
import { TextCommand } from 'src/lib/text_commands/textCommand';
import { TextCommandHandler } from '../../lib/text_commands/textCommandHandler';
...
export function useEnhancedInput<
    T extends HTMLInputElement | HTMLTextAreaElement,
>(props: UseEnhancedInputProps<T>) {
    const { value, onChange, name, inputRef, onKeyDown, textCommands } = props;
    ...
    const textCommandHandler = useRef(new TextCommandHandler(textCommands));
    ...
    useEffect(() => {
        ...
        if (selectedIndex >= 0 && listItemRefs.current[selectedIndex]) {
            ...
            if (
                ...
                scrollIndex = (selectedIndex - forwardScroll + nItems) % nItems;
            }
